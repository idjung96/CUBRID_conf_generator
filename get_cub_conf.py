...
def generate_broker_acl_file(setup_info):
    ...
    for b_item in setup_info['broker_info']:
        ...
        ip_list = setup_info['pc_ip'].split(';')
        for ip in ip_list:
            f.write(ip + '\n')
        f.close()

        f = open(os.path.join(b_item['location'], 'wasip.txt'), 'a+')
        ip_list = setup_info['app_ip'].split(';')
        for ip in ip_list:
            f.write(ip + '\n')
        f.close()
...
def is_valid_conf(setup_info):
    ...
    if 'svc_name' not in setup_info:
        print('[ERR] MUST have \'svc_name\' key')
        return False

    if 'cubrid_home' not in setup_info:
